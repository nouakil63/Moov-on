...
(() => {
  ...
  function openManual(){
    const id=crypto.randomUUID();openDialog('Ajouter une activité',`<form id="manual-form"><label for="manual-sport">Sport</label><select id="manual-sport"><option>Course</option><option>Marche</option><option>Vélo</option></select><div class="form-grid"><div><label for="manual-distance">Distance (mètres)</label><input id="manual-distance" type="number" value="5000" min="1" max="1000000" required></div><div><label for="manual-duration">Durée (minutes)</label><input id="manual-duration" type="number" value="30" min="0.1" max="10080" step="0.1" required></div></div><label for="manual-title">Titre</label><input id="manual-title" value="Ma sortie" maxlength="160"><label class="privacy-check"><input id="manual-hide" type="checkbox" checked> Cacher mon trajet</label><p class="micro-note">Une activité saisie manuellement ne contient aucun trajet.</p><label class="privacy-check"><input id="manual-publish" type="checkbox" checked> Publier les statistiques dans le fil de mon entreprise</label><div class="calculation-preview" id="manual-preview" aria-live="polite"></div><button class="btn impact block">Enregistrer l’activité</button></form>`);
    const input=()=>({id,sport:$('manual-sport').value,distanceMeters:Number($('manual-distance').value),durationSeconds:Number($('manual-duration').value)*60,title:$('manual-title').value,hideRoute:$('manual-hide').checked,publish:$('manual-publish').checked});
    const preview=()=>{try{const calc=Energy.calculate(input(),P().rules());$('manual-preview').innerHTML=flame()+' <b>'+n(calc.energy)+' énergie</b><small>'+decimal(calc.speedKmh)+' km/h · bonus '+decimal(calc.bonusPct)+' %</small>';}catch(e){$('manual-preview').textContent='Renseignez une distance et une durée valides.';}};
    $('manual-form').addEventListener('input',preview);preview();$('manual-form').onsubmit=e=>{e.preventDefault();try{const saved=P().recordActivity(input());dialog.close();goHome();render();if(saved.published)revealActivity(saved.id);toast('Activité enregistrée · '+n(saved.energy)+' énergie · '+euro(saved.contributionCents)+' mobilisés.');}catch(err){dialogError(err);}};
  }
  ...
  window.MoovApp={init,render,stop,toast,goHome,openStart,openManual,openCreateEvent,openStory:()=>window.MoovStories?.openComposer(),fastForward(){if(run.active&&!run.paused){run.durationSeconds+=1000/run.speedKmh*3.6;run.distanceMeters=run.durationSeconds*run.speedKmh/3.6;updateRun();}},activityContext};
  ...
  document.querySelectorAll('[data-fan]').forEach(b=>b.onclick=()=>{closeFan();({run:openStart,result:openManual,story:()=>window.MoovStories?.openComposer(),event:openCreateEvent}[b.dataset.fan])();});
