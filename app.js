...
(() => {
  ...
  const calendarIcon=()=>icon('<rect x="3" y="5" width="18" height="16" rx="2"/><path d="M8 3v4M16 3v4M3 11h18M8 15h2M14 15h2"/>');
  ...
  function homeCampaign(stats){
    ...
    const event=P().events().find(e=>e.startsAt>Demo.now());
    ...
    const campaign=s?`<button class="defi3" data-impact aria-label="Voir la campagne solidaire"><div class="d3-row"><span class="d3-em">${flame()}</span><div><h3>${esc(s.campaign.name||s.mission?.name)}</h3><p>Notre campagne · encore ${Math.ceil(s.remainingDays)} jours</p></div></div><div class="d3-bar"><i style="width:${percent}%"></i><span class="d3-dot" style="left:${Math.max(4,Math.min(96,percent))}%">${icon('<path d="m5 12 4 4L19 6"/>',11)}</span></div><div class="d3-cap num"><span>${n(s.financedImpact)} / ${n(s.targetImpact)} ${esc(unit)}</span><span>${Math.round(percent)} %</span></div></button>`:`<button class="defi3" data-impact><div class="d3-row"><span class="d3-em">${flame()}</span><div><h3>Notre prochaine mission arrive</h3><p>Votre entreprise prépare sa campagne.</p></div></div></button>`;
    const meeting=`<button class="defi3 event-invitation" data-home-events><div class="d3-row"><span class="d3-em">${calendarIcon()}</span><div><h3>${event?esc(event.name):'Retrouvons-nous pour bouger'}</h3><p>${event?esc(eventDate(event.startsAt))+' · '+eventTime(event.startsAt):'Découvrez les événements du collectif'}</p></div></div><div class="d3-cap"><span>${event?n(event.distanceMeters)+' mètres · '+event.participantIds.length+' inscrit(s)':'Entreprise, ouverts à tous ou sur invitation'}</span><span>${arrowIcon()}</span></div></button>`;
  ...
  function renderHome(){
    ...
    $('home-campaign').innerHTML=homeCampaign(activeStats());$('home-campaign').querySelectorAll('[data-impact]').forEach(b=>b.onclick=()=>showScreen('scr-missions'));$('home-campaign').querySelector('[data-home-events]').onclick=()=>showScreen('scr-events');
